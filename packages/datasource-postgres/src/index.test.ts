...
const url = 'postgres://user:pass@localhost:5432/myapp';
...
describe('createPostgresDatasource', () => {
  it('resolves a key to a connection string with the per-branch database', () => {
    const datasource = createPostgresDatasource({ url, prefix: 'app' });
  ...
  it('keeps host and credentials while swapping the database, whatever the base url points at', () => {
    const perBranch = createPostgresDatasource({ url: 'postgres://user:pass@localhost:5432/app_old__fp' });
    expect(perBranch.resolve('new__fp')).toBe('postgres://user:pass@localhost:5432/app_new__fp');
  });

  it('declares instant-clone and isolation capability', () => {
    const datasource = createPostgresDatasource({ url });
  ...
  it('creates a database with a quoted name', async () => {
    ...
    await createPostgresDatasource({ url, prefix: 'app', query }).create('main__abc');
    expect(calls[0]?.sql).toBe('CREATE DATABASE "app_main__abc"');
  });

  it('clones a database using TEMPLATE', async () => {
    const { query, calls } = recordingRunner(noRows);
    await createPostgresDatasource({ url, prefix: 'app', query }).clone('a__fp', 'b__fp');
    expect(calls[0]?.sql).toBe('CREATE DATABASE "app_b__fp" TEMPLATE "app_a__fp"');
  });

  it('destroys a database if it exists', async () => {
    const { query, calls } = recordingRunner(noRows);
    await createPostgresDatasource({ url, prefix: 'app', query }).destroy('main__abc');
  ...
  it('reports existence based on returned rows', async () => {
    ...
    expect(await createPostgresDatasource({ url, query: present.query }).exists('main__abc')).toBe(true);
    const absent = recordingRunner(noRows);
    expect(await createPostgresDatasource({ url, query: absent.query }).exists('main__abc')).toBe(false);
  });
  ...
  it('lists provisioned keys by stripping the prefix', async () => {
    ...
    expect(await createPostgresDatasource({ url, prefix: 'app', query }).list()).toEqual(['main__abc', 'dev__fp']);
  });
